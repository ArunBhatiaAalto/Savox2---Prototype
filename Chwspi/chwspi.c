...
const int clockRate = 3600000;
const int channels[CHN_AMOUNT] = {0, 1, 2, 3};
const int threshold = 450; // Sound level threshold
    ...
    for (int i = 0; i < blocks; i++) {
        for (int j = 0; j < CHN_AMOUNT; j++) {
            ...
            */
            ...
            // ads7861 single output  4 channel, M0 = 1 M1 = 1, MOSI connected to convst and RD
            // cs_change 0 at all times
            //tx[(i * CHN_AMOUNT + j) * 4] = 1 << 7;
            
            //ads8688 (len 4)
            //tx[(i * CHN_AMOUNT + j) * 4] = 0xC | (j << 2);
            //tx[(i * CHN_AMOUNT + j) * 4 + 1] = 1; //voltage mode
...
int *spi_transfer(struct spi_ioc_transfer* spitr, int fd, unsigned char *rx, unsigned char *tx, int *buf) {
    ...
    for (int i = 0, j = 0; i < CHN_AMOUNT * blocks; i++, j += 4) {
        ...
        // ads7861
        //int chn = (rx[j] & 0b01100000) >> 5
        //buf[chn] = ((int) (rx[j] & 0x1F) << 7) + (rx[j + 1] >> 1);

        // ads8688
        //buf[i] = (rx[j + 2] << 8) + rx[j + 3];
